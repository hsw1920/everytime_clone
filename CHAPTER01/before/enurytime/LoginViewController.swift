...
final class LoginViewController: UIViewController{
    ...
    private let idTextField: InsetTextField = {
        ...
        textField.insetX = 16 // 아이디 비밀번호와 rect간 space를 두기위해 InsetTextField가 UITextField를 상속하여 사용한다.
        textField.translatesAutoresizingMaskIntoConstraints = false
        textField.placeholder = "아이디"
        textField.font = UIFont.systemFont(ofSize: 14, weight: .regular)
        textField.backgroundColor = UIColor(w: 242)
        textField.layer.cornerRadius = 20
        textField.clipsToBounds = true // true 적용해야 radius가 정상적으로 적용됨
        return textField
    }()
    ...
    override func viewDidLoad() {
        ...
        NSLayoutConstraint.activate([
            ...
            self.idTextField.leadingAnchor.constraint(equalTo: self.container.leadingAnchor, constant: 40),
            self.idTextField.trailingAnchor.constraint(equalTo: self.container.trailingAnchor, constant: -40),
            
            self.passwordTextField.heightAnchor.constraint(equalToConstant: 40),
            self.passwordTextField.leadingAnchor.constraint(equalTo: self.container.leadingAnchor, constant: 40),
            self.passwordTextField.trailingAnchor.constraint(equalTo: self.container.trailingAnchor, constant: -40),
            
            self.loginButton.heightAnchor.constraint(equalToConstant: 40),
            self.loginButton.leadingAnchor.constraint(equalTo: self.container.leadingAnchor, constant: 40),
            self.loginButton.trailingAnchor.constraint(equalTo: self.container.trailingAnchor, constant: -40),
